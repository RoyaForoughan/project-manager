...
class AuthController{
    ...
    async login(req,res, next){
        try {
            ...
            if(!user) throw {status : 401 , message : 'نام کاربر ی یا رمز عبور اشتباه می باشد'}
            const compareResult = bcrypt.compareSync(password , user.password)
            if(!compareResult) throw {status : 401 , massage : 'نام کاربری یا رمز عبور اشتباه می باشد'}
